...
def follow_faces(robot: cozmo.robot.Robot):
    ...
    objface_to_follow = None

    print("Press CTRL-C to quit")
    while True:
        turn_action = None
        if objface_to_follow:
            # start turning towards the face
            # この1文に、上下左右の移動が埋められている、はず。
            turn_action = robot.turn_towards_face(objface_to_follow)

        if not (objface_to_follow and objface_to_follow.is_visible):
            # find a visible face, timeout if nothing found after a short while
            try:
                objface_to_follow = robot.world.wait_for_observed_face(timeout=30)
...
# 画像表示の記載が特にないのは、「use_viewer=True」だけで表示されるようになるから？
